...
def trigger_compliance_run(
    ...
    """
    Run a full CE v3.2 assessment and save results to compliance history.
    Results are retained for 12 months per CE v3.2 / IASME requirements.
    Rate limit: one concurrent run per tenant.
    """
    ...
    def _do_run() -> None:
        ...
        run_db = SessionLocal()
        try:
            save_compliance_run(run_db, tenant_id, triggered_by="user")
            try:
                from services.compliance_service import assess_tenant
                result = assess_tenant(run_db, tenant_id)
                controls = result.controls if hasattr(result, "controls") else []
                for _c in controls:
                    if getattr(_c, "status", "") not in ("fail", "partial"):
                        continue
                    _event = {
                        "event_type": "ce_control_failure",
                        "severity": 8 if getattr(_c, "status", "") == "fail" else 5,
                        "description": f"CE v3.2 control {getattr(_c, 'control_id', '')} failed: {getattr(_c, 'control_name', '')}",
                        "ce_control": getattr(_c, "control_id", ""),
                        "ce_compliant": False,
                        "remediation_class": "approval_required" if getattr(_c, "status", "") == "fail" else "informational",
                        "remediation_action": (getattr(_c, "remediation", "") or "")[:300],
                        "tenant_id": tenant_id,
                    }
                    _asyncio.run(_dispatch_ce(run_db, tenant_id, _event))
            except Exception as _exc:
                import logging as _l; _l.getLogger(__name__).warning("CE dispatch failed: %s", _exc)
        finally:
            run_db.close()
            _run_in_progress.discard(tenant_id)

    background_tasks.add_task(_do_run)
    return {"status": "run_started", "tenant_id": tenant_id}
