...
# ── Network identity enrichment helpers ───────────────────────────────
_CONSUMER_VENDOR_MAP = {
    "apple": ("mobile_device", "Apple / iPhone / iPad / Watch"),
    "iphone": ("mobile_device", "Apple / iPhone / iPad / Watch"),
    "ipad": ("mobile_device", "Apple / iPhone / iPad / Watch"),
    "watch": ("wearable", "Smart Watch / Wearable"),
    "samsung": ("mobile_device", "Samsung Mobile / Smart Device"),
    "xiaomi": ("mobile_device", "Xiaomi Mobile / Smart Device"),
    "huawei": ("mobile_device", "Huawei Mobile / Smart Device"),
    "oppo": ("mobile_device", "Android Mobile Device"),
    "oneplus": ("mobile_device", "Android Mobile Device"),
    "google": ("smart_device", "Google Home / Chromecast / Android"),
    "roku": ("smart_tv", "Roku / Streaming Device"),
    "sonos": ("smart_device", "Smart Speaker / Audio Device"),
    "ring": ("ip_camera", "Smart Security / Doorbell"),
    "hikvision": ("ip_camera", "IP Camera / CCTV"),
    "dahua": ("ip_camera", "IP Camera / CCTV"),
    "tp-link": ("network_device", "Router / Booster / Extender"),
    "netgear": ("network_device", "Router / Booster / Extender"),
    "asus": ("network_device", "Router / Booster / Extender"),
    "ubiquiti": ("network_device", "Network Infrastructure"),
    "linksys": ("network_device", "Router / Booster / Extender"),
    "murata": ("smart_device", "IoT / TV / Mobile Radio Module"),
    "liteon": ("laptop", "Laptop / Consumer Device"),
    "canon": ("printer", "Printer / Imaging Device"),
    "epson": ("printer", "Printer / Imaging Device"),
    "brother": ("printer", "Printer / Imaging Device"),
    "hp": ("printer", "Printer / Imaging Device"),
    "xerox": ("printer", "Printer / Imaging Device"),
    "ricoh": ("printer", "Printer / Imaging Device"),
    "synology": ("nas_device", "NAS / Storage"),
    "qnap": ("nas_device", "NAS / Storage"),
    "drobo": ("nas_device", "NAS / Storage"),
    "lg": ("smart_tv", "Smart TV"),
    "sony": ("smart_tv", "Smart TV"),
}


def _infer_asset_confidence(asset: object) -> str:
    meta = getattr(asset, "raw_metadata_json", None) or {}
    existing = meta.get("asset_confidence")
    if existing in {"confirmed_asset", "likely_asset", "observed_host"}:
        return existing
    has_mac = bool(getattr(asset, "mac_address", None))
    has_name = any(bool(getattr(asset, f, None)) for f in ("hostname", "netbios_name", "mdns_name", "fqdn"))
    has_ports = bool(getattr(asset, "open_ports", None)) or bool(getattr(asset, "services", None))
    has_vendor = bool(getattr(asset, "vendor", None))
    if has_mac or has_name or has_ports:
...
def _infer_asset_identity(asset: object) -> tuple[str, str, str | None, float | None, str | None, bool]:
    vendor = (getattr(asset, "vendor", None) or "").strip()
    device_type = getattr(asset, "device_type", None) or "unknown"
    device_family = getattr(asset, "device_family", None)
    risk_score = getattr(asset, "risk_score", None)
    risk_level = getattr(asset, "risk_level", None) or getattr(asset, "risk_hint", None)
    is_rogue = bool(getattr(asset, "is_rogue", False))
    conf = _infer_asset_confidence(asset)

    open_ports = getattr(asset, "open_ports", None) or []
    ports = []
    for p in open_ports:
        try:
            ports.append(int(p.get("port")) if isinstance(p, dict) else int(p))
        except Exception:
            pass

    service_names = []
    for s in (getattr(asset, "services", None) or []):
        if isinstance(s, dict):
            nm = (s.get("service") or s.get("name") or "").lower().strip()
            if nm:
                service_names.append(nm)

    names_blob = " ".join(filter(None, [
        str(getattr(asset, "hostname", "") or ""),
        str(getattr(asset, "netbios_name", "") or ""),
        str(getattr(asset, "mdns_name", "") or ""),
        str(getattr(asset, "fqdn", "") or ""),
        vendor,
        str((getattr(asset, "http_headers", None) or {}).get("title", "")),
        str((getattr(asset, "http_headers", None) or {}).get("server", "")),
    ])).lower()

    if vendor:
        for needle, mapped in _CONSUMER_VENDOR_MAP.items():
            if needle in vendor.lower():
                if device_type in {None, "", "unknown", "iot_device", "smart_device"}:
                    device_type = mapped[0]
                if not device_family:
                    device_family = mapped[1]
                break

    if any(k in names_blob for k in ["iphone", "ipad", "android", "galaxy", "pixel"]):
        device_type = "mobile_device"
        device_family = device_family or "Mobile / Phone / Tablet"
    elif any(k in names_blob for k in ["watch", "wearable"]):
        device_type = "wearable"
        device_family = device_family or "Smart Watch / Wearable"
    elif any(k in names_blob for k in ["roku", "chromecast", "bravia", "smarttv", "webos", "tizen"]):
        device_type = "smart_tv"
        device_family = device_family or "Smart TV / Streaming Device"
    elif any(k in names_blob for k in ["printer", "epson", "hp", "brother", "xerox", "ricoh"]):
        device_type = "printer"
        device_family = device_family or "Printer / Imaging Device"
    elif any(k in names_blob for k in ["extender", "booster", "repeater", "mesh"]):
        device_type = "network_device"
        device_family = device_family or "Router / Booster / Extender"

    if device_type in {"unknown", ""}:
        if any(p in ports for p in [135, 139, 445, 3389]):
            device_type = "windows_workstation"
            device_family = device_family or "Windows Workstation"
        elif any(p in ports for p in [515, 631, 9100]):
            device_type = "printer"
            device_family = device_family or "Printer / Imaging Device"
        elif any(p in ports for p in [53, 1900]) or any(sn in service_names for sn in ["upnp", "ssdp"]):
            device_type = "network_device"
            device_family = device_family or "Router / Booster / Extender"
        elif 443 in ports and len(ports) <= 2:
            device_type = "web_server"
            device_family = device_family or "Web Server / Appliance"

    if conf == "observed_host":
        if vendor or device_type not in {None, "", "unknown"}:
            conf = "likely_asset"
        if getattr(asset, "mac_address", None) or ports or any(bool(getattr(asset, f, None)) for f in ("hostname", "netbios_name", "mdns_name", "fqdn")):
            conf = "confirmed_asset"

    unmanaged = not bool(getattr(asset, "managed", False) or getattr(asset, "agent_installed", False))
    if unmanaged and conf == "confirmed_asset" and (vendor or ports or device_type not in {None, '', 'unknown'}):
        if device_type in {"network_device", "ip_camera", "smart_device", "nas_device", "smart_tv"}:
            is_rogue = True

    return conf, (device_type or "unknown"), device_family, risk_score, risk_level, is_rogue


def _network_assets_summary(items: list[dict]) -> dict:
    out = {"total": len(items), "managed": 0, "rogue": 0, "critical": 0, "high": 0, "open_ports": 0, "confirmed": 0, "likely": 0, "observed": 0, "mobile": 0, "smart_tv": 0, "iot": 0}
    for a in items:
        if a.get("managed") or a.get("agent_installed"):
            out["managed"] += 1
        if a.get("is_rogue"):
            out["rogue"] += 1
        level = str(a.get("risk_level") or a.get("risk_hint") or "").upper()
        if level == "CRITICAL": out["critical"] += 1
        if level == "HIGH": out["high"] += 1
        out["open_ports"] += len(a.get("open_ports") or [])
        conf = a.get("asset_confidence")
        if conf == "confirmed_asset": out["confirmed"] += 1
        elif conf == "likely_asset": out["likely"] += 1
        else: out["observed"] += 1
        dt = a.get("device_type") or ""
        if dt == "mobile_device": out["mobile"] += 1
        elif dt == "smart_tv": out["smart_tv"] += 1
        elif dt in {"smart_device", "ip_camera", "wearable", "printer", "nas_device"}: out["iot"] += 1
    return out
...
def run_network_scan(
    ...
    Thread(
        target = _worker,
        args   = (job_id, payload.tenant_id, payload.target, payload.requested_by),
        daemon = True,
    ).start()
        ...
        items.append(
            {
                ...
                "message": progress.get("msg", ""),
                "hosts_found": summary.get("discovered_count", 0),
                "assets_found": summary.get("discovered_count", 0),
                "critical_hosts": risk_breakdown.get("critical", risk_breakdown.get("CRITICAL", 0)),
                "high_hosts": risk_breakdown.get("high", risk_breakdown.get("HIGH", 0)),
                "error": summary.get("error") or summary.get("nmap_error"),
...
def get_scan_progress(
    ...
    return {
        ...
        "engine": job.engine or summary.get("engine") or "pending",
        "error": summary.get("error") or summary.get("nmap_error"),
        "diagnostics": summary.get("diagnostics", {}),
    }
...
def _detect_local_subnet() -> str:
    ...
    Returns a /24 CIDR based on the primary outbound interface IP.
    Falls back to 127.0.0.0/24 only if detection completely fails.
    """
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        # No traffic is actually sent; this is a common way to discover
...
def _resolve_scan_target(raw_target: Optional[str]) -> str:
    """
    Use UI-provided CIDR if present; otherwise auto-detect local subnet.
    """
    target = (raw_target or "").strip()
    if not target:
        return _detect_local_subnet()

    # Accept either CIDR or single IP.
    try:
        if "/" in target:
            return str(ipaddress.ip_network(target, strict=False))
        # Single host entered: normalise to /32
...
def run_network_scan(
    ...
    resolved_target = _resolve_scan_target(payload.target)
    ...
    def _worker(jid: int, tenant_id: str, target: str, requested_by: str | None):
        ...
        try:
            ...
            try:
                failed = (
                    worker_db.query(NetworkScanJob)
                    .filter(NetworkScanJob.id == jid)
                    .first()
                )
                if failed:
                    failed.status = "failed"
...
@router.post("/reclassify-assets")
def reclassify_network_assets(
    auth: AuthenticatedRequest = Depends(require_admin),
    db: Session = Depends(get_db),
) -> dict:
    """Recalculate confidence/device family and merge passive discovery hints."""
    passive_by_ip = {}
    passive_by_mac = {}
    try:
        from models.network_extensions import PassiveDiscoveryResult
        passive_rows = (
            db.query(PassiveDiscoveryResult)
            .filter(PassiveDiscoveryResult.tenant_id == auth.tenant_id)
            .order_by(PassiveDiscoveryResult.id.desc())
            .limit(500)
            .all()
        )
        for p in passive_rows:
            if getattr(p, "ip_address", None) and p.ip_address not in passive_by_ip:
                passive_by_ip[p.ip_address] = p
            mac = (getattr(p, "mac_address", None) or "").lower()
            if mac and mac not in passive_by_mac:
                passive_by_mac[mac] = p
    except Exception:
        pass

    rows = db.query(NetworkDiscoveredAsset).filter(NetworkDiscoveredAsset.tenant_id == auth.tenant_id).all()
    updated = 0
    for r in rows:
        passive = None
        mac = (getattr(r, "mac_address", None) or "").lower()
        if mac and mac in passive_by_mac:
            passive = passive_by_mac[mac]
        elif getattr(r, "ip_address", None) in passive_by_ip:
            passive = passive_by_ip[getattr(r, "ip_address")]
        if passive:
            if not getattr(r, "mac_address", None) and getattr(passive, "mac_address", None):
                r.mac_address = passive.mac_address
            if not getattr(r, "vendor", None) and getattr(passive, "vendor", None):
                r.vendor = passive.vendor
            if not any([getattr(r, 'hostname', None), getattr(r, 'netbios_name', None), getattr(r, 'mdns_name', None)]) and getattr(passive, 'hostname', None):
                r.hostname = passive.hostname
            meta = dict(getattr(r, 'raw_metadata_json', None) or {})
            meta['passive_source_method'] = getattr(passive, 'source_method', None)
            meta['passive_confidence'] = getattr(passive, 'confidence', None)
            r.raw_metadata_json = meta

        conf, dtype, dfam, _, rlevel, rogue = _infer_asset_identity(r)
        r.device_type = dtype
        if dfam:
            r.device_family = dfam
        r.is_rogue = rogue
        if rlevel and not getattr(r, 'risk_level', None):
            r.risk_level = rlevel
        meta = dict(getattr(r, 'raw_metadata_json', None) or {})
        meta['asset_confidence'] = conf
        meta['identity_reclassified_at'] = 'manual'
        r.raw_metadata_json = meta
        updated += 1
