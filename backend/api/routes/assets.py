...
from services.nvd_service import get_latest_scan_run
...
def list_assets(
    ...
    if agent_ids:
        ...
        latest_vuln_scan = get_latest_scan_run(db, auth.tenant_id)
        if latest_vuln_scan:
            cve_rows = (
...
def list_unified_assets_route(
    ...
    unified_cve_counts = {}
    latest_vuln_scan = get_latest_scan_run(db, auth.tenant_id)
    if real_agent_ids and latest_vuln_scan:
        cve_rows = (
            db.query(VulnerabilityFinding.agent_id, func.count(VulnerabilityFinding.id))
            .filter(
                VulnerabilityFinding.tenant_id == auth.tenant_id,
    ...
    for item in raw_assets:
        ...
        if is_real_agent:
            ...
            authoritative_cve_count = unified_cve_counts.get(agent_lookup_id, 0)
