...
def _cvss_to_severity(score: float | None, nvd_label: str) -> str:
    """
    NVD sometimes returns incorrect baseSeverity labels (e.g. CVSS 10.0 as HIGH).
    Enforces correct NVD v3 bands: 9.0-10.0=CRITICAL, 7.0-8.9=HIGH, 4.0-6.9=MEDIUM, 0.1-3.9=LOW
    """
    if score is None:
        return (nvd_label or "UNKNOWN").upper()
    ...
    for item in data.get("vulnerabilities", []):
        ...
        for key in ("cvssMetricV31", "cvssMetricV30", "cvssMetricV2"):
            ...
            if entries:
                ...
                raw_severity = cvss_data.get("baseSeverity", entries[0].get("baseSeverity", "UNKNOWN"))
                severity = _cvss_to_severity(cvss_score, raw_severity)
                break
        description = ""
