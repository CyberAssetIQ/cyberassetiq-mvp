...
Asset sources:
  - CanonicalAsset    (agent-managed, rich OS/posture data)
  - NetworkDiscoveredAsset (agentless scan, ip/ports/CVE data)
"""
...
def _overall_status(controls: list[ControlResult]) -> tuple[float, str]:
    ...
    # CE requires all controls to pass — any FAIL = overall FAIL
...
def assess_tenant(db: Session, tenant_id: str) -> dict[str, Any]:
    """
    Run CE v3.2 assessment for ALL assets in a tenant:
    - CanonicalAsset (agent-managed, rich data)
    - NetworkDiscoveredAsset (agentless, is_active=True, no agent installed)

    Returns a unified tenant-level summary with per-asset breakdown.
    """
    reports: list[AssetComplianceReport] = []

    # --- Agent-managed assets ---
    agent_assets = (
        db.query(CanonicalAsset)
    ...
    # --- Network-discovered assets (active, no agent installed) ---
    # Exclude devices that already have an agent (already covered above)
    net_assets = (
        db.query(NetworkDiscoveredAsset)
        .filter(
            NetworkDiscoveredAsset.tenant_id == tenant_id,
            ...
            NetworkDiscoveredAsset.asset_confidence != "observed_host",  # exclude ping-only ghosts
        )
        .all()
    )
    ...
    if not reports:
        return {
            ...
            "assessed_at_epoch": int(time.time()),
            "assets_assessed": 0,
            "agent_assets_assessed": 0,
            "network_assets_assessed": 0,
            "assets_passing": 0,
            "assets_partial": 0,
            "assets_failing": 0,
            "ce_ready": False,
    ...
    agent_reports = [r for r in reports if r.asset_source == "agent"]
    network_reports = [r for r in reports if r.asset_source == "network"]
    passing_assets = [r for r in reports if r.overall_status == "PASS"]

    return {
        "tenant_id": tenant_id,
        "assessed_at_epoch": int(time.time()),
        "assets_assessed": len(reports),
        "agent_assets_assessed": len(agent_reports),
        "network_assets_assessed": len(network_reports),
        "assets_passing": len(passing_assets),
        "assets_partial": sum(1 for r in reports if r.overall_status == "PARTIAL"),
        "assets_failing": sum(1 for r in reports if r.overall_status == "FAIL"),
        "ce_ready": len(passing_assets) == len(reports) and len(reports) > 0,
        "tenant_overall_score": round(
            sum(r.overall_score for r in reports) / len(reports), 3
        ),
        "assets": [
            {
                "agent_id": r.agent_id,
...
def save_compliance_run(
    ...
    run = ComplianceRun(
        ...
        agent_assets_assessed=result.get("agent_assets_assessed", 0),
        network_assets_assessed=result.get("network_assets_assessed", 0),
