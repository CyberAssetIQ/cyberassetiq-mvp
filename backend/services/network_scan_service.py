...
from services.merge_service import sync_network_discoveries_batch
...
def run_network_scan_job(
    ...
    try:
        ...
        # Push saved network evidence into canonical inventory so the sales demo
        # shows one trusted asset record instead of disconnected scan rows.
        try:
            sync_count = sync_network_discoveries_batch(
                db,
                tenant_id=tenant_id,
                network_assets=stored,
            )
            print(f"[network_scan] synced {sync_count} network assets into canonical inventory")
        except Exception as sync_exc:
            logger.warning("[scan] canonical sync failed: %s", sync_exc)
            db.rollback()
