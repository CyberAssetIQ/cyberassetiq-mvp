...
def _managed_contexts(db: Session, tenant_id: str) -> dict[str, ManagedContext]:
    ...
    .filter(
        ...
        CanonicalAsset.agent_id.like("agent-%"),
...
def _correlate_findings_metadata(db: Session, tenant_id: str) -> None:
    ...
    latest_network_scan_job_id = db.query(func.max(NetworkDiscoveredAsset.scan_job_id)).filter(
        NetworkDiscoveredAsset.tenant_id == tenant_id,
        NetworkDiscoveredAsset.scan_job_id.isnot(None),
    ).scalar()

    network_query = db.query(NetworkDiscoveredAsset).filter(
        NetworkDiscoveredAsset.tenant_id == tenant_id
    )

    if latest_network_scan_job_id is not None:
        network_query = network_query.filter(
            NetworkDiscoveredAsset.scan_job_id == latest_network_scan_job_id
        )

    network_assets = network_query.all()
...
def invalidate_unified_cache(tenant_id: str) -> None:
    """Call this whenever assets are created/updated/deleted to bust the 60-second cache."""
    _unified_cache.pop(tenant_id, None)
    _unified_cache_ts.pop(tenant_id, None)
...
def list_unified_assets(db: Session, tenant_id: str, refresh_darkweb_metadata: bool = False) -> list[dict[str, Any]]:
    ...
    # ── Manual assets ────────────────────────────────────────────────────
    manual_assets = db.query(ManualAsset).filter(
        ManualAsset.tenant_id == tenant_id,
        ManualAsset.is_deleted.is_(False),
    ).all()
    for m in manual_assets:
        ip_list = [m.ip] if m.ip else []
        unified.append(
            {
                "asset_uid": f"manual-{m.id}",
                "display_name": m.hostname or m.ip or f"manual-{m.id}",
                "managed": False,
                "source": "manual",
                "source_types": ["manual"],
                "source_count": 1,
                ...
                "open_cve_count": 0,
                "critical_cve_count": 0,
                "linked_darkweb_findings": [],
                "darkweb_findings_count": 0,
                "last_seen_epoch": None,
                "compliance_score": None,
                "compliance_status": "NOT_ASSESSED",
                "security_posture": {},
